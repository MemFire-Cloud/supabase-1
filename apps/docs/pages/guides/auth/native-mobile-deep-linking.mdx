...
在项目的 [auth settings](https://supabase.com/dashboard/project/_/auth/url-configuration)中添加重定向 URL，例如 `com.supabase://***`。

最后，实现 OAuth 和链接处理程序。有关在 React Native 中初始化 supabase-js 客户端的说明，请参阅 [supabase-js 参考资料](/docs/reference/javascript/initializing?example=react-native-options-async-storage)。
...
### 深度链接配置
- 转到 [auth settings](https://supabase.com/dashboard/project/_/auth/url-configuration) 页面。
...
1. 进入 [auth settings](https://supabase.com/dashboard/project/_/auth/url-configuration) 页面。
2. 在 "附加重定向 URL "字段中输入应用程序重定向 URL。这是用户点击魔法链接后重定向到的 URL。
